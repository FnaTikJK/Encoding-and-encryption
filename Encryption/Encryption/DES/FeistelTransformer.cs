...
public static class FeistelTransformer
{
    public static bool[] Transform(bool[] bits)
    {
        var l = bits[..32];
        var r = bits[32..];
        for (var i = 0; i < 16; i++)
        {
            var key = KeyGenerator.Keys[i];
            (l, r) = (
                r, 
                l.Xor(FeistelFunc(key, r))
            );
        }
    ...
    public static bool[] ReverseTransform(bool[] bits)
    {
        var l = bits[..32];
        var r = bits[32..];
        for (var i = 0; i < 16; i++)
        {
            var key = KeyGenerator.Keys[15 - i];
            (r, l) = (
                l,
                r.Xor(FeistelFunc(key, l))
            );
        }
    ...
    private static bool[] FeistelFunc(bool[] key, bool[] halfBits)
    {
        ...
        var sTransformed = expanded
            .Chunk(6)
            .SelectMany(STransform)
            .ToArray(); // size is 4 * 8 = 32
        return sTransformed.Shuffle(PShuffle);
    }
    
    private static readonly int[] ExpandTo48BitsTable =
    {
        32, 1, 2, 3, 4, 5,
        4, 5, 6, 7, 8, 9,
        8, 9, 10, 11, 12, 13,
    ...
    private static bool[] STransform(bool[] sixBits, int index)
    {
        var a = new[] {sixBits.First(), sixBits.Last()}.ToByte();
        var b = sixBits[1..5].ToByte();
        var @byte = STables[index][a][b];
        return @byte.ToBoolArray(4);
    }
    
    private static readonly byte[][][] STables =
    {
        new []
        {
    ...
    private static readonly int[] PShuffle =
    {
