...
public static class Rid_Salomon
{
    ...
    {
        var correctionBytesCount = CorrectionBytesByCorrection[correction][version - 1];
        var generationPolinomial = GenerationPolinomial[correctionBytesCount];
        var result = new byte[Math.Max(correctionBytesCount, block.Length)];
        block.CopyTo(result, 0);
        for (int i = 0; i < block.Length; i++)
        {
            var a = result[0];
            result.MoveValuesLeft();
            if (a == 0)
                continue;

            var b = ReversedGaluaField[a];
            for (var j = 0; j < generationPolinomial.Length; j++)
            {
                var v = (byte)((b + generationPolinomial[j]) % 255);
                result[j] ^= GaluaField[v];
            }
        }
    ...
    private static byte[] ReversedGaluaField =
    {
